...
import com.viifo.tgvideocompress.audio_input.AudioInput;
import com.viifo.tgvideocompress.audio_input.BlankAudioInput;
import com.viifo.tgvideocompress.audio_input.GeneralAudioInput;
...
public class MediaCodecVideoConvertor {
    ...
    private boolean convertVideoInternal(ConvertVideoParams convertVideoParams, boolean increaseTimeout, int triesCount) {
        ...
        // final boolean isWebm = false; //  = convertVideoParams.isSticker;
        // boolean shouldUseHevc = false; // = isStory;
        ...
        try {
            ...
            long currentPts = 0;
            float durationS = duration / 1000f; // 秒
            ...
            if (videoIndex >= 0 && !MediaController.VIDEO_MIME_TYPE.equals(extractor.getTrackFormat(videoIndex).getString(MediaFormat.KEY_MIME))) {
...
//                        }
                        ...
                        outputFormat.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, 20); // 原始值 1
                        ...
                        outputSurface.changeFragmentShader(
                                createFragmentShader(originalWidth, originalHeight, resultWidth, resultHeight, true, 3),
                                createFragmentShader(originalWidth, originalHeight, resultWidth, resultHeight, false, 3),
                                false
                        );
...
//                        }
                        ...
                        if (audioIndex >= 0) {
                            ...
                            if ("audio/unknown".equals(audioFormat.getString(MediaFormat.KEY_MIME))) {
                                audioIndex = -1;
                            }
                                    ...
                                    if (info.size > 0 && (endTime < 0 || info.presentationTimeUs < endTime)) {
                                        ...
                                        if (availableSize != 0) {
                                            if (callback != null) {
                                                ...
                                                float progress = ((currentPts / 1000f / 1000f) / durationS) * 100;
                                                callback.didWriteData(availableSize, progress);
                                            }
                                        }
                                    }
                                } else if (index == -1) {
